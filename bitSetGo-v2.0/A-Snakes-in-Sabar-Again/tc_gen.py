...
def main():
    ...
    for i in XRange(0, test_files + 1):
        ...
        required_input = max(5, POWER(10, i//2 + 1))
        print(required_input)  # Prints x into input file
        for _ in range(required_input):
            l = RINT(4, 26)
            m = RINT(4, POWER(10, i//3 + 2))
            n = RINT(4, POWER(10, i//3 + 2))
            print(l, m, n, sep = " ")
            a = random.sample(range(1, 27), l)
            a = sorted(a)
            for ch in a:
                print(chr(96 + ch), end  = " ")
            print()
            b = random.sample(range(1, POWER(10, i//3 + 2) + 1), m)
            b = sorted(b)
            for nu in b:
                print(nu, end = " ")
            print()
            c = random.sample(range(1, POWER(10, i//3 + 2) + 1), n)
            c = sorted(c)
            for nu in c:
                print(nu, end = " ")
            print()
